...
export default {
  ...
  data: () => ({
    ...
    chartOptions: {
     ...
     title: {
        display: true,
        text: 'Who Caught the Most Villains?'
      }
    }
  }),
  async mounted () {
    this.loaded = false
    try {
      const response = await axios.get('http://localhost:8080/caught')
      const responseData = response.data
      this.chartData.datasets[0].data = responseData.map(entry => entry.catchersCount);
